...
.\linebreak
```{r glgraph, echo = FALSE, fig.align = 'center', out.width = "600px", fig.cap="Função densidade de probabilidade de cinco distribuições de qui-quadrado com diferentes graus de liberdade."} 
library("ggplot2")
ggplot(data.frame(x = c(0, 15)), aes(x = x)) + 
  stat_function(fun = dchisq, args = list(df = 1), aes(colour = "gl = 1"), lwd = 1) +
  stat_function(fun = dchisq, args = list(df = 2), aes(colour = "gl = 2"), lwd = 1) +
  stat_function(fun = dchisq, args = list(df = 3), aes(colour = "gl = 3"), lwd = 1) +
  stat_function(fun = dchisq, args = list(df = 5), aes(colour = "gl = 5"), lwd = 1) +
  stat_function(fun = dchisq, args = list(df = 9), aes(colour = "gl = 9"), lwd = 1) +
  labs(x = "Qui-quadrado", y = "Densidade de probabilidade") + 
  scale_color_manual(name = "Graus de\nLiberdade", values = c("firebrick2", "chocolate", "royalblue3", "green4", "deepskyblue3")) +
...
O teste exato de Fisher [Fisher 1922](https://doi.org/10.2307/2340521) é válido para todos os tamanhos de amostra, e se torna exato no limite à medida que o tamanho da amostra cresce até o infinito, como acontece com muitos testes estatísticos.
...
### Exercício 1 -- Traça de tigre escarlate
...
bb <- c(5)
N <- c(1612)
data09_1 <- data.frame(Genotipo,BB,Bb,bb,N)
knitr::kable(data09, align = "lcccc", booktabs = TRUE, caption = "Polimorfismo de cor na asa da traça de tigre escarlate
(_Callimorpha dominula_).", "html")
```
.\linebreak

### Exercício 2 -- Urso de Kermode
...
GG <- c(21)
N <- c(132)
data09_2 <- data.frame(Genotipo,AA,AG,GG,N)
knitr::kable(data09, align = "lcccc", booktabs = TRUE, caption = "Frequências dos genótipos para o polimorfismo MC1R em populações das ilhas da Colúmbia Britânica do urso de Kermode (_Ursus americanus kermodei_).", "html")
```
.\linebreak
